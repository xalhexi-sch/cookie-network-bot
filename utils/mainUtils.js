...
async function saveTranscriptTxt(interaction, ticketChannel, user = null) {
  let channel = null;
  if (interaction) channel = interaction.channel;
  else if (ticketChannel) channel = ticketChannel;

  if (!channel) {
    console.warn(`[saveTranscriptTxt]: No channel provided or accessible for transcript.`);
    return null;
  }
  ...
  let transcript = [];
  let totalFetched = 0;
  // ... following code continues unchanged
...
async function lastUserMsgTimestamp(userId, channelId) {
  ...
  if (!channel) {
    console.warn(`[lastUserMsgTimestamp]: Channel ${channelId} not found or inaccessible.`);
    return null;
  }
  ...
  while (true) {
    ...
    try {
      ...
      console.error(`[lastUserMsgTimestamp]: Failed to fetch messages for ${channelId}:`, err);
      break;
    }
...
async function lastChannelMsgTimestamp(channelId) {
    ...
    // ✅ Stop immediately if channel doesn't exist or can't be fetched
    if (!channel) {
        console.warn(`[lastChannelMsgTimestamp]: Channel ${channelId} not found or inaccessible.`);
        return null;
    }
    ...
    while (true) {
        ...
        try {
            ...
            console.error(`[lastChannelMsgTimestamp]: Failed to fetch messages for ${channelId}:`, err);
            break;
        }
        ...
        for (const msg of fetched.values()) {
            if (!lastTimestamp || msg.createdTimestamp > lastTimestamp) {
                lastTimestamp = msg.createdTimestamp;
            }
        }
